...
def tournamentWinner(competitions, results):
    ...
    for i in range(len(results)):
        checkValue = competitions[i][abs((results[i]-1))]
        
        if checkValue in calculatedResults:
            calculatedResults[checkValue] += 3
        else:
            calculatedResults[checkValue] = 3 

    return max(calculatedResults,key=calculatedResults.get)
